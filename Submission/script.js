...
const getBotReply = (msg) => {
  ...
  if (userInput === "reset") {
    level = 1;
    return `Hi there ${userName}, so you need help deciding what anime you should watch. Lucky for you that's my speciality! Have you watched any anime before?`;
  }
  if (userInput === "help") {
  ...
  if (level === 0) {
    if (userName === undefined) userName = userInput;
    level = 1;
    path = 0;
    return `Hi there ${userName}, so you need help deciding what anime you should watch. Lucky for you that's my speciality! Have you watched any anime before?`;
  }
  ...
  if (path === 1) {
    if (level === 2) {
      ...
      if (noReplys.includes(userInput)) {
        return "I would highly recommend Death Note. An extremely worthwhile watch, the protagonist Light Yagami finds an ominous notepad at school one day and discovers his new purpose.";
      }
    }
  }
  if (path === 2) {
    if (level === 2) {
      ...
      if (noReplys.includes(userInput)) {
        return "Fantastic! I recommend Attack on Titan, main character Eren Yeager has lived inside the walls his entire life, one fateful day everything changed.";
      }
    }
  }
  if (level === 3) {
    if (yesReplys.includes(userInput)) {
      return "Excellent, I suggest you watch Code Geass. The main character Lelouch vi Britannia battles against the odds as he fights to regain control of his home country.";
    }
    if (noReplys.includes(userInput)) {
      return "Seeing as you didn't enjoy Death Note so much, I'm going to suggest something that sets a completely different pace. That time I was reincarnated as a slime.";
    }
  }
  if (level === 4) {
    if (yesReplys.includes(userInput)) {
